...
train <- HEM %>% 
  filter(Metadata_matrix %in% c("FN","HEM")) %>%
  ungroup() %>% 
  select_(.dots = feature_cols_aggregated)

tsne <- Rtsne(train, perplexity = 10, verbose = TRUE, max_iter = 500, theta = .5, pca_scale = TRUE,initial_dims = 100)
...
Y <- as.data.frame(tsne$Y)
  ...
  df = cbind(as.data.frame(Y),
             as.data.frame(aggregated %>% filter(Metadata_matrix %in% c("FN","HEM"))))
